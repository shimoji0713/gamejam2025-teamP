...
*マクロ定義
*************************/
#define RANKING_FILE     ("Resource/image/renkingdata.csv")
...
* 戻り値：なし
*********************************/
AbstractScene* Ranking:: Update(void)
{
	switch (DispMode)
	{
	case RANKING_INPUT_MODE:
	...
	case RANKING_DISP_MODE:
	default:
		if (input->OnButton(XINPUT_BUTTON_B))
		{
			return new Title;
...
void ranking_input_name(void)
{
	...
	//カーソル操作処理
	if (input->OnButton(XINPUT_BUTTON_DPAD_LEFT) == TRUE)
	{
		if (Cursor.x > 0)
		{
			Cursor.x--;
	...
	if (input->OnButton(XINPUT_BUTTON_DPAD_RIGHT) == TRUE)
	{
		if (Cursor.x < 12)
		{
			Cursor.x++;
	...
	if (input->OnButton(XINPUT_BUTTON_DPAD_UP) == TRUE)
	{
		if (Cursor.y > 0)
		{
			Cursor.y--;
	...
	if (input->OnButton(XINPUT_BUTTON_DPAD_DOWN) == TRUE)
	{
		if (Cursor.y < 4)
		{
			Cursor.y++;
	...
	//文字を選択する
	if (input->OnButton(XINPUT_BUTTON_B) == TRUE)
	{
		if (Cursor.y < 2)
		{
